...
- For Device App (Page files), either manually set the widgets, or manipulate the state object:
  ...
  ```
- For the Settings App, `this.state` is a build-time cache only - mutating it in callbacks does **not** update the screen. Every callback write to `settingsStorage` must represent real app state (not a dummy revision key). Hydrate `this.state` from storage at the start of `build` (`getStorage`).

  Official recommended structure:
  1. Define `state` (`this.state`)
  2. At the beginning of `build`, read `settingsStorage` into `state`
  3. Program logic
  4. Return the render tree
  5. In event callbacks, change `settingsStorage` to re-run `build` and refresh the UI
